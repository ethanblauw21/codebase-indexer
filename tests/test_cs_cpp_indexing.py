...
real symbols, but incremental_indexer's INDEXABLE_EXTS historically omitted their
source extensions, so the disk scan never fed them — C#/C++ code was never chunked
(only .csproj/.sln descriptors, edges-only). These tests lock the scan gate to the
Tier-A claim so the regression can't silently return.
"""
import os
...
def test_cs_cpp_source_extensions_are_scanned():
    for ext in (".cs", ".cpp", ".cc", ".cxx", ".h", ".hpp"):
        assert ext in ii.INDEXABLE_EXTS, f"{ext} must be chunk/embed-scanned (Tier A)"


def test_project_descriptors_stay_edges_only():
    # .csproj/.sln remain descriptor-only (chunking would embed XML noise, not code).
    assert ".csproj" in ii.PROJECT_EXTS and ".csproj" not in ii.INDEXABLE_EXTS
    assert ".sln" in ii.PROJECT_EXTS and ".sln" not in ii.INDEXABLE_EXTS
