...
**Status:** accepted (2026-07-02) — built + merged (PR #12). Both ADR-009 decisions settled: reranker (C−B) and convex fusion (D−B) both FAIL the §5 bar → both flags stay off. The B−A graph gate is **formally dropped** per §8 below (the graph layer is recorded as rerank-only, not a first-pass retrieval signal); making it retrieval-competitive is deferred to ADR-022.
**Date:** 2026-06-22
...
- [~] Stand up the §6 private slice: author a **freshly-written clean-room repo** (post-cutoff, ~2 languages, real call edges) + git-ignored fixtures; document how to run it locally. **NOW THE LIVE GATE (2026-07-06)** — was "moot" while both public verdicts said *don't enable*; the n=148 power rerun changed that. The reranker C−B now clears clauses 1 & 2 on the public eval, so clause 3 (private confirmation) is **the one remaining bar** between the reranker and `enabled=true`. Authoring the clean-room slice is the next concrete step to settle §P4. *(Convex fusion D−B was not rerun and stays rejected — this promotion is reranker-only.)*
