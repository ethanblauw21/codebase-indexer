...
- [ ] Implement three-state verdicts (VERIFIED/ADVISORY/INSUFFICIENT) + safe-direction rule in `analyze_blast_radius`, `find_dead_code`, `detect_pattern_violations`
- [ ] `verify_candidate_edges` snippet tool over `DocumentStore` (no resolution logic); optional reranker pre-sort
