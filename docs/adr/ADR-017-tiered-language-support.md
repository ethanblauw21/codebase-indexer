...
- [ ] Implement three-state verdicts (VERIFIED/ADVISORY/INSUFFICIENT) + safe-direction rule in `analyze_blast_radius`, `find_dead_code`, `detect_pattern_violations` — **moved to the retriever-unification PR (Item 2 / "Piece 5")**
- [ ] `verify_candidate_edges` snippet tool over `DocumentStore` (no resolution logic); optional reranker pre-sort — **moved to the retriever-unification PR (§7.1 verdict machinery)**
