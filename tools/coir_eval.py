...
# ---------------------------------------------------------------------------
# Reranker (§8) — cross-encoder rescoring of the dense top-k candidates.
#
# Qwen3-Reranker is NOT a sentence-transformers CrossEncoder: it is a causal LM
# that judges (query, doc) relevance by reading the logit of the "yes" vs "no"
# token after an instruction prompt (Qwen's documented interface). We wrap it
# behind a .predict(pairs, batch_size) method so run_subtask() drives it exactly
# like a CrossEncoder. load_reranker() picks the right loader by model id, so
# swapping back to a true CrossEncoder later needs no call-site changes.
# ---------------------------------------------------------------------------
